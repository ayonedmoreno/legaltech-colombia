...
Navegador ──► Next.js (apps/web) ──HTTP/cookie──► API Fastify (apps/api)
   ...
   (+ pgvector)    (privado)       (pg-boss)     (paquete puro)  (paquete puro)  (adaptador)
                                        │
                                        ▼
...
- **Base de datos:** PostgreSQL y Prisma, IDs UUID, `pgvector` para embeddings (Fase 7), dinero en enteros. Las consultas de similitud y algunos índices irán con SQL crudo y migraciones manuales.
...
La estructura detallada por carpeta figura en el plan del Sprint 1A.

---

## 4. Dependencias

Las dependencias concretas y sus versiones se fijan en el Sprint 1A (verificando la versión vigente al instalar) y se registran en el `package.json` de cada workspace. Criterios:
